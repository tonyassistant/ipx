...
fn signal_lines(iface: &NetworkInterface) -> Vec<Line<'static>> {
    ...
    vec![
        ...
        bullet_line("Probe-backed validation for private-route and upstream-configured certainty".to_string()),
    ]
}
...
fn reachability_style(state: ReachabilityState) -> Style {
    match state {
        ReachabilityState::UpstreamConfigured => Style::default().fg(Color::Green),
